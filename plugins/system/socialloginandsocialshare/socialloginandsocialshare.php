  ...
  function onAfterInitialise() {
	...
	if ($obj->IsAuthenticated == true && JFactory::getUser()->id) {
	  $lrdata = plgSystemSocialLoginTools::get_userprofile_data($userprofile);
	   $query = "SELECT provider from #__LoginRadius_users WHERE provider=".$db->Quote ($lrdata['Provider'])." AND id = " . JFactory::getUser()->id;
	        $db->setQuery($query);
	  ...
	  if ($lr_settings ['dummyemail'] == 1 && $lrdata['email'] == '') {
        ...
        if (isset($user_id)) {
		  ...
		  if ($useractivation == '0') {
			...
			$msgtitle = $lr_settings['Popup_Email_Title'];
			$msg = $lr_settings['Popup_Email_Message'];
			...
			plgSystemSocialLoginTools::enterEmailPopup($msg, $msgtitle, $msgtype);
		   }
	     }
       }
	 }
	...
	if (isset($_POST['sociallogin_emailclick'])) {
	  ...
	  if (isset($_POST['session']) && $_POST['session'] == $lrdata['session'] && !empty($lrdata['session'])) {
		...
		$msg = $lr_settings['Popup_Error_Email_Message'];
		$msgtitle = $lr_settings['Popup_Email_Title'];
		$msgtype = 'warning';
        if(!JMailHelper::isEmailAddress($email)) {
	      ...
	      if ($user_exist != 0 ) {
	        ...
	        $lrdata = $session->get('tmpuser');
            $email = $db->escape(urldecode($_POST['email']));
            $lrdata['email'] = $email;
	      }
	    }
	  }
 ...
 */
   ...
   function _sendMail(&$user, $usermessgae) {
    ...
    elseif ($lr_settings['sendemail'] == 1  && $usermessgae == 2) {
      $db = JFactory::getDBO();
